...
*Now we _can_ add an additional step (this is optional for expected values!):*
...
# we repeat the response function from above,
# but store the result as lambda (to follow the notation of the formul)
lambda1 <- exp(Xbeta1)
lambda2 <- exp(Xbeta2)
lambda3 <- exp(Xbeta3)
...
Let's see how we could simulate predicted values. For illustration purposes, we will only do this for autocracies.

To get predicted values, we want to draw from the stochastic component of the model. For negative binomial models, this means drawing from a negative binomial distribution. This has two parameter: the mean ($\lambda$) and the scale paramater ($\theta$).

```{r}
# We already have the mean parameter. Those are our expected values from above.
# Let's get the scale parameter theta:
theta <- m2$theta

# draw predicted values
pv_auto <- rnbinom(nsim, size = theta, mu = exp_auto)

# Let's see how the resulting distribution looks like:
hist(pv_auto, breaks = max(pv_auto),
     main = "1000 predicted values of one-sided killings in Autocracie",
     xlab = "Predicted values",
     las = 1)

# Most of the times, the model predicts zero one-sided killings.
# Let's zoom in a little bit to better see the rest of the distribution.
# -> We reduce the height of the y-axis
...
m4 <-glm.nb(
    os_best ~ intensity_dyad + auto + demo + prior_os,
    data = dta[dta$govt == 0,], # data subsetted to rebel killings
    control = glm.control(maxit = 100)
  )
...
scenario1 <- cbind(1, median(dta$intensity_dyad, na.rm = T), 1, 0, mean(dta$prior_os, na.rm = T))
scenario2 <- cbind(1, median(dta$intensity_dyad, na.rm = T), 0, 1, mean(dta$prior_os, na.rm = T))
scenario3 <- cbind(1, median(dta$intensity_dyad, na.rm = T), 0, 0, mean(dta$prior_os, na.rm = T))
...
# To get expected values, we need to plug in the Xbeta values into the response function
exp_scenario1_m3 <- exp(Xbeta1_m3)
exp_scenario2_m3 <- exp(Xbeta2_m3)
exp_scenario3_m3 <- exp(Xbeta3_m3)
...
# To get expected values, we need to plug in the Xbeta values into the response function
exp_scenario1_m4 <- exp(Xbeta1_m4)
exp_scenario2_m4 <- exp(Xbeta2_m4)
exp_scenario3_m4 <- exp(Xbeta3_m4)

exp_values_m4 <- cbind(exp_scenario1_m4, exp_scenario2_m4, exp_scenario3_m4)
...
# Autocracy - Democracy
df_m3$fd_auto_dem <- df_m3$X1 - df_m3$X2
# Autocracy - Anocracy
df_m3$fd_auto_ano <- df_m3$X1 - df_m3$X3

# Model 4: Killings by Rebel Groups
# Autocracy - Democracy
df_m4$fd_auto_dem <- df_m4$X1 - df_m4$X2
# Autocracy - Anocracy
df_m4$fd_auto_ano <- df_m4$X1 - df_m4$X3
```
...
ci_fd_auto_dem_m3 <- quantile(df_m3$fd_auto_dem, c(0.025, 0.975))

# Autocracy - Anocracy, M3 (by Government)
median_fd_auto_ano_m3 <- median(df_m3$fd_auto_ano)
ci_fd_auto_ano_m3 <- quantile(df_m3$fd_auto_ano, c(0.025, 0.975))

# Autocracy - Democracy, M4 (by Rebel Groups)
median_fd_auto_dem_m4 <- median(df_m4$fd_auto_dem)
ci_fd_auto_dem_m4 <- quantile(df_m4$fd_auto_dem, c(0.025, 0.975))

# Autocracy - Democracy, M4 (by Rebel Groups)
median_fd_auto_ano_m4 <- median(df_m4$fd_auto_ano)
ci_fd_auto_ano_m4 <- quantile(df_m4$fd_auto_ano, c(0.025, 0.975))
```
